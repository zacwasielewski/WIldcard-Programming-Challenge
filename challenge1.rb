...
def get_cols( grid )
  grid_to_array( grid ).transpose
end

def possible_positions( row_or_col, spaces_required )
  row_or_col.map{ |row|
    row.select{ |cell| cell == OPEN_POSITION }.permutation(spaces_required).count
  }.inject(:+)
end

row_positions = possible_positions(get_rows(GRID), SPACES_REQUIRED)
col_positions = possible_positions(get_cols(GRID), SPACES_REQUIRED)

puts row_positions + col_positions
